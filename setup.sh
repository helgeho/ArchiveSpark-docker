...
if [ ! -d /config/spark ]; then
    curl -L -O http://archive.apache.org/dist/spark/spark-2.1.3/spark-2.1.3-bin-hadoop2.7.tgz
...
if [ ! -d /config/toree ]; then
    curl -L -O https://archive.org/~helge/ArchiveSpark/toree2_dynamic_lib.tar.gz
...
if [ ! -d /config/lib ]; then
    mkdir /config/lib
    curl -L -O https://github.com/helgeho/ArchiveSpark/releases/download/v.3.0/archivespark-assembly-3.0.jar
    curl -L -O https://github.com/helgeho/ArchiveSpark/releases/download/v.3.0/archivespark-assembly-3.0-deps.jar
